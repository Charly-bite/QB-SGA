    ...
    with e.connect() as conn:
        with conn.begin():
            ...
            for pid, evs in product_events.items():
                # Read existing JSON
                res = conn.execute(text("SELECT lote_history FROM product_classifications WHERE product_id = :pid"), {"pid": pid})
                rows = res.fetchall()
                ...
                if added:
                    # Sort descending
                    hist.sort(key=lambda x: str(x.get('date') or x.get('timestamp') or ''), reverse=True)
                    conn.execute(text("UPDATE product_classifications SET lote_history = :hist WHERE product_id = :pid"), 
