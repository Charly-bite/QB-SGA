...
class SGAVisorApp:
    def __init__(self, root):
        ...
        # Procesos Frame
        frame_proc = tk.LabelFrame(root, text="Servidores y Rogue Process en Puerto 5000", padx=10, pady=10, font=("Arial", 11, "bold"))
        frame_proc.pack(fill="both", expand=True, padx=10, pady=10)
        
        self.tree = ttk.Treeview(frame_proc, columns=("PID", "Name", "Details"), show="headings", height=8)
        ...
        tk.Button(btn_frame, text="Refrescar Lista", command=self.refresh_procs, bg="#f0f0f0").pack(side="left", padx=5)
        tk.Button(btn_frame, text="Terminar Proceso Seleccionado", command=self.kill_selected, bg="#ffd966").pack(side="left", padx=5)
        tk.Button(btn_frame, text="Liberar todo el Puerto 5000 (Purga Total)", command=self.kill_all_5000, bg="#ff4d4d", fg="white", font=("Arial", 9, "bold")).pack(side="right", padx=5)
        
        # Watchdog Frame
        frame_wd = tk.LabelFrame(root, text="Control del Watchdog", padx=10, pady=10, font=("Arial", 11, "bold"))
        frame_wd.pack(fill="x", padx=10, pady=10)
        
        self.wd_status = tk.StringVar()
        self.wd_status.set("Estado del Watchdog: Desconocido")
        
        self.status_label = tk.Label(frame_wd, textvariable=self.wd_status, font=("Arial", 12, "bold"))
        self.status_label.pack(pady=10)
        
        wd_btn_frame = tk.Frame(frame_wd)
        wd_btn_frame.pack(fill="x", pady=5)
        tk.Button(wd_btn_frame, text="Iniciar Watchdog", command=self.start_watchdog, bg="#99ff99", font=("Arial", 10, "bold")).pack(side="left", fill="x", expand=True, padx=10)
        tk.Button(wd_btn_frame, text="Detener Watchdog", command=self.stop_watchdog, bg="#ff9999", font=("Arial", 10, "bold")).pack(side="left", fill="x", expand=True, padx=10)
    ...
    def get_port_5000_pids(self):
        try:
            ...
            output = subprocess.check_output('netstat -aon | findstr ":5000 "', shell=True).decode(errors='ignore')
            pids = set()
    ...
    def get_process_info(self, pid):
        try:
            cmd = f'wmic process where processid="{pid}" get name,commandline /format:csv'
    ...
    def refresh_procs(self):
        ...
        if not pids:
            self.tree.insert("", "end", values=("-", "Ninguno", "Puerto 5000 libre. Todo 정상."))
            return
    ...
    def kill_all_5000(self):
        answer = messagebox.askyesno("Confirmar", "¿Seguro que quieres matar todos los procesos en el puerto 5000?")
    ...
    def stop_watchdog(self):
        try:
            ...
            subprocess.call(cmd_batch, shell=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
            ...
            subprocess.call(cmd_py, shell=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
            
            # (Por si acaso quedó la ventana abierta) Mata la ventana de CMD que se llama "SGA Server Watchdog"
            os.system('taskkill /F /FI "WINDOWTITLE eq SGA Server Watchdog*" /T >nul 2>&1')
            time.sleep(1)
            self.check_watchdog_status()
        except Exception as e:
            messagebox.showerror("Error", str(e))
