...
class SettingsManager:
    ...
    def __init__(self, config_file: str, barcode_db_file: str = None, excel_db_path: str = None):
        """
        Args:
        ...
        if self.sql_engine:
            try:
                import pandas as pd
                df_vars = pd.read_sql("SELECT * FROM product_variants", con=self.sql_engine)
    ...
    def _load_peso_tara_from_excel(self, excel_path: str):
        ...
        try:
            ...
            for _, row in df.iterrows():
                ...
                if codigo_variante and codigo_variante != 'nan' and pd.notna(codigo_variante):
                    self.peso_tara_map[codigo_variante] = float(peso_tara) if pd.notna(peso_tara) else 0.0
            ...
            print(f"Loaded {len(self.peso_tara_map)} variants with Peso Tara data from Excel")
    ...
    def apply_overrides(self, product_data: Dict[str, Any], father_id: str, variant_id: str = None) -> Dict[str, Any]:
        """
