...
for e in lote_events:
            ...
            if pid:
                for hist in changes['lote_history']:
                    ...
                    entries_to_insert.append({
                        ...
                        'date': hist.get('date') or hist.get('timestamp') or e.get('timestamp'),
    ...
    for entry in entries_to_insert:
        ...
        cursor.execute("SELECT id FROM product_lote_history WHERE product_id = ? AND event_date = ?", (pid, ts))
        ...
        if str(old_date).lower() == 'nan': old_date = None
        if str(new_date).lower() == 'nan': new_date = None
        ...
        try:
            ...
            if not exists_in_sql:
                cursor.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, (pid, old_lote, new_lote, old_date, new_date, old_reinsp_date, new_reinsp_date, ts, user_name, merma_kg, notes))
        except Exception as ex:
            pass # Probably already exists
            
        try:
            # 2. Append to product_classifications lote_history
            cursor.execute("SELECT lote_history FROM product_classifications WHERE product_id = ?", (pid,))
            row = cursor.fetchone()
            if row and row[0]:
                import json
                try:
            ...
            if not exists:
                ...
                cursor.execute("UPDATE product_classifications SET lote_history = ? WHERE product_id = ?", (json.dumps(hist), pid))
                conn.commit()
                inserted += 1
