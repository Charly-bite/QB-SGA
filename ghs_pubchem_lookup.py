...
PICTOGRAM_MAP = {
    ...
    'GHS08': ['Health Hazard', 'peligro para la salud', 'health hazard', 'mutagénico', 'carcinógeno'],
...
H_TO_PICTOGRAM = {
    ...
    'H228': 'GHS02', 'H241': 'GHS02', 'H242': 'GHS02', 'H250': 'GHS02',
    'H251': 'GHS02', 'H252': 'GHS02', 'H260': 'GHS02', 'H261': 'GHS02',
    # GHS03 - Oxidizers
    'H270': 'GHS03', 'H271': 'GHS03', 'H272': 'GHS03',
    # GHS04 - Compressed gases
    'H280': 'GHS04', 'H281': 'GHS04', 'H282': 'GHS04', 'H283': 'GHS04', 'H284': 'GHS04',
    # GHS05 - Corrosives
    ...
    'H362': 'GHS08', 'H370': 'GHS08', 'H371': 'GHS08', 'H372': 'GHS08', 'H373': 'GHS08',
    # GHS09 - Environmental
    'H400': 'GHS09', 'H401': 'GHS09', 'H402': 'GHS09',
    'H410': 'GHS09', 'H411': 'GHS09', 'H412': 'GHS09', 'H413': 'GHS09', 'H420': 'GHS09',
}

# Signal word determination from H codes
DANGER_H_CODES = {
    'H200', 'H201', 'H202', 'H203', 'H204', 'H205',
    'H220', 'H224', 'H225', 'H240', 'H241', 'H250', 'H260', 'H270', 'H271',
    'H280', 'H281',
    'H300', 'H301', 'H310', 'H311', 'H314', 'H318',
    'H330', 'H331', 'H334',
    'H340', 'H341', 'H350', 'H351', 'H360', 'H361', 'H370', 'H372',
    'H304',
}
...
class PubChemLookup:
    ...
    def parse_ghs_response(self, ghs_json):
        ...
        try:
            ...
            ghs_section = self._find_section(ghs_json.get('Record', {}), 'GHS Classification')
            if not ghs_section:
                return result
            ...
            for info in ghs_section.get('Information', []):
                ...
                for sv in value.get('StringWithMarkup', []):
                    ...
                    h_match = re.match(r'^(H\d{3})\s*(?:\*+\s*)?(?:\([^)]*\)\s*)?:', text)
                    if h_match:
...
class DatabasePopulator:
    ...
    def populate(self, results):
        ...
        existing_pic = {(r['product_id'], r['pictogram_id']) for r in product_pictograms}
                ...
                for sub_h in h_code.split('+'):
                    ...
                    if (pid, sub_h) not in existing_h:
                        ...
                        if sub_h not in known_h_codes:
                            logger.warning(f"    ⚠️ H code {sub_h} not in h_statements.csv (add description manually)")
            ...
            for p_code in result.get('p_codes', []):
                if (pid, p_code) not in existing_p:
                    ...
                    if p_code not in known_p_codes:
                        logger.warning(f"    ⚠️ P code {p_code} not in p_statements.csv (add description manually)")
        ...
        if not self.dry_run:
            ...
            self._write_csv('product_hazards.csv', all_hazards, ['product_id', 'h_code'])
            self._write_csv('product_precautions.csv', all_precautions, ['product_id', 'p_code'])
            self._write_csv('product_pictograms.csv', all_pictograms, ['product_id', 'pictogram_id'])
            
            # Update products_master
            master_fields = list(products_master[0].keys()) if products_master else []
            self._write_csv('products_master.csv', list(products_by_id.values()), master_fields)
...
def run_report():
    ...
    skip_cas = {'Mezcla', 'No aplicable', 'No Aplicable', '', 'Mixture', '-', None, 'Sustancia'}
    ...
    print(f"  Genuinely non-hazardous (approx):    {len(products) - len(has_cas) - parseable}")
    print("=" * 70)
    print()
    print("  Strategy:")
...
def run_full_lookup(dry_run=False):
    ...
    for p in products:
        ...
        if result and (result['h_codes'] or result['p_codes'] or result['pictograms']):
            ...
            legacy_count += 1
            logger.info(f"  ✅ {result['product_id']}: {len(result['h_codes'])} H, "
    ...
    if REQUESTS_AVAILABLE:
        ...
        for p in products:
            ...
            if pc_result and (pc_result.get('h_codes') or pc_result.get('pictograms')):
                ...
                if pid in already_resolved:
                    ...
                    for existing in results:
                        if existing['product_id'] == pid:
                            ...
                            if not existing['signal_word'] and pc_result.get('signal_word'):
                ...
                logger.info(f"  ✅ {pid}: {len(pc_result['h_codes'])} H, "
                           f"{len(pc_result['p_codes'])} P, {len(pc_result['pictograms'])} pics")
            else:
                logger.info(f"  ⚪ {pid}: No GHS classification in PubChem (may be non-hazardous)")
...
Examples:
  ...
  python ghs_pubchem_lookup.py                  # Run full lookup & populate DB
    ...
    parser.add_argument('--cas', type=str,
                       help='Test lookup for a single CAS number')
    parser.add_argument('--report', action='store_true',
                       help='Show current missing data report')
