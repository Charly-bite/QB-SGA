...
for pid in sorted(lote_updates.keys()):
    ...
    print(f"  {pid}: lote={info['lote']}, elab={info['lote_date']}, reinsp={info['lote_reinspection_date']}, by={info['user']} @ {info['timestamp']}")
...
if os.path.exists(prod_class_path):
    ...
    prod_with_lote = {pid: d for pid, d in prod_class.items() if str(d.get('lote', '') or '').strip()}
...
for h in sorted(all_history_entries, key=lambda x: x['date'])[-20:]:
    print(f"  {h['product_id']}: {h['old_lote']} -> {h['new_lote']} by {h['user']} @ {h['date']}")
