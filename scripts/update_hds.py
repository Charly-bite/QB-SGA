...
# Database columns
DB_HEADERS = [
    'Codigo interno', 'Chemical Name', 'CAS Number', 'Signal Word',
    'Emergencia', 'H-Statements', 'Consejos (Frases P)', 'Source_File',
    'Exclamación', 'Corrosión', 'Peligro para la salud', 'Calavera',
    'Llama', 'Ambiente', 'Llama sobre círculo', 'Bomba explotando',
    'Cilindro de gas'
]
...
H_TO_PICTOGRAM = {
    # Exclamación (GHS07)
    'Exclamación': {
        'H302', 'H303', 'H312', 'H313', 'H315', 'H316', 'H317',
        'H319', 'H320', 'H332', 'H333', 'H335', 'H336', 'H402', 'H412',
    },
    # Corrosión (GHS05)
    'Corrosión': {
        'H290', 'H314', 'H318',
    ...
    'Llama': {
        ...
        'H227', 'H228', 'H241', 'H242', 'H250', 'H251', 'H252',
        'H260', 'H261',
    },
    # Ambiente (GHS09)
    'Ambiente': {
...
def run_update(dry_run=True):
    ...
    changes = []  # List of (row_idx, field, old_value, new_value, product_name, codigo, hds_file)
    ...
    for product in products:
        ...
        for ridx in db_lookup[codigo]:
            ...
            source = row[source_idx].strip() if source_idx >= 0 and len(row) > source_idx else ''
            ...
            if sw_idx >= 0 and len(row) > sw_idx:
                ...
                if (db_signal_norm != hds_signal_norm and 
                    hds_signal_norm != 'no aplicable' and
                    db_signal not in ('', '-')):
                    changes.append({
                        'row_idx': ridx,
                        'col_idx': sw_idx,
            ...
            if h_idx >= 0 and len(row) > h_idx and hds_h:
                ...
                if missing_in_db:
                    ...
                    changes.append({
                        ...
                        'col_idx': h_idx,
                        'field': 'H-Statements',
                        'old': db_h_raw[:80] + ('...' if len(db_h_raw) > 80 else ''),
                        'new': new_h_str,
                        'nombre': nombre,
                        'codigo': codigo,
                        'hds_file': Path(hds_file).name,
    ...
    if not dry_run:
        print("\n[3/4] Creating backup...")
        backup_path = DATABASE_CSV.with_suffix(f'.csv.backup_{datetime.now().strftime("%Y%m%d_%H%M%S")}')
