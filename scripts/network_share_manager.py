...
class NetworkShareManager:
    ...
    def __init__(self, config_file: str = "shared_config.json"):
        ...
        self.mount_path = self.config.get('network', {}).get('mount_path', '/mnt/sga_shared')
    ...
    def print_diagnostics(self):
        ...
        print(f"   ✓ Mount Point Exists: {'YES' if status['mount_point_exists'] else 'NO'}")
        ...
        if status['config_valid']:
            print(f"   ✓ Server Reachable: {'YES' if status['server_reachable'] else 'NO'}")
...
"""
    ...
    def generate_credentials_template(self, output_file: str = "sga-credentials.template"):
...
Examples:
  ...
  %(prog)s --generate-scripts    Create mount script and credentials template
    ...
    parser.add_argument('--config', default='shared_config.json',
