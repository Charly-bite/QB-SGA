...
class SAPHanaConnector:
    ...
    def __init__(self, host: str = None, port: int = None, username: str = None, password: str = None, schema: str = None):
        """
    ...
    def connect(self, username: str = None, password: str = None) -> bool:
        ...
        if not user or not pwd:
            raise ValueError("Username and password are required for SAP HANA connection")
        ...
        try:
            ...
            logger.error(f"[ERROR] Fatal error establishing SAP connection: {str(e)}", exc_info=True)
            self._local.connection = None
            self._local.connected = False
            return False
    ...
    def test_connection(self) -> Dict[str, Any]:
        ...
        if not self.connected:
            try:
                ...
                raise ConnectionError("Not connected to SAP HANA and auto-connection failed")
        ...
        try:
            ...
            material_count = 0
            try:
                cursor.execute(f"SELECT COUNT(*) FROM {self._get_table_name('materials')}")
    ...
    def get_materials(self, language: str = 'S', limit: int = 1000) -> pd.DataFrame:
        ...
        return pd.read_sql(query, getattr(self._local, 'connection', None), params=[language])
    
    def get_material_by_code(self, material_number: str, language: str = 'S') -> Optional[Dict]:
        """
        ...
        if row:
            columns = ['material_number', 'material_type', 'material_group', 
                      'base_unit', 'gross_weight', 'net_weight', 'description']
            return dict(zip(columns, row))
        return None
    
    def search_materials(self, search_term: str, language: str = 'S', limit: int = 50) -> pd.DataFrame:
        """
        ...
        return pd.read_sql(query, getattr(self._local, 'connection', None), params=[language, search_pattern, search_pattern])
...
# End of file (Watchdog trigger)
    ...
    def get_order_details(self, order_number: int) -> Optional[Dict]:
        ...
        try:
            ...
            logger.warning(f"Could not fetch invoice number for order {order_number}: {e}")
            factura_number = None
        ...
        header = {
            ...
            'creator_name': header_row[12] if len(header_row) > 12 and header_row[12] else 'SAP System',
            'updater_name': header_row[13] if len(header_row) > 13 and header_row[13] else 'SAP System'
        }
    ...
    def get_batch_dates(self, item_code: str, batch_number: str = None) -> Optional[Dict]:
        """
        ...
        try:
            ...
            if row:
                ...
                logger.info(f"[BATCH] Batch dates for {item_code}: mfg={result['manufacturing_date']}, exp={result['expiry_date']}")
                return result
    ...
    def get_all_latest_batches(self, item_codes: List[str] = None) -> Dict[str, Dict]:
        ...
        try:
            ...
            if item_codes:
                ...
                for i in range(0, len(item_codes), chunk_size):
                    ...
                    query = base_query.replace(
                        '{ITEM_FILTER}',
                        f'AND B."ItemCode" IN ({placeholders})'
                    )
                    cursor = getattr(self._local, 'connection', None).cursor()
                    cursor.execute(query, chunk)
                    for row in cursor.fetchall():
            ...
            logger.info(f"[BATCH] Bulk batch sync: fetched latest batch for {len(results)} items")
            return results
    ...
    def get_item_weights(self, item_code: str) -> Dict[str, float]:
        ...
        try:
            ...
            logger.warning(f"Could not fetch item weights from SAP for {item_code}: {e}")
    ...
    def get_item_ghs_data(self, item_code: str) -> Optional[Dict]:
        ...
        try:
            ...
            logger.info(f"[GHS] UDF data for {item_code}: signal='{signal_word}', pictos={pictograms}")
            return result

        except Exception as e:
            logger.warning(f"Could not fetch GHS UDF data from SAP for {item_code}: {e}")
            return None
    ...
    def get_item_batches(self, item_code: str, warehouse: str = None) -> List[Dict]:
        ...
        try:
            ...
            logger.info(f"[BATCH] Found {len(batches)} batches for {item_code} whs={warehouse or 'ALL'}")
            return batches
    ...
    def sync_products_from_sap(self, limit: int = 2000, updated_since: str = None) -> Dict:
        """
        ...
        try:
            ...
            cols = ['item_code', 'item_name', 'foreign_name', 'item_group',
                    'sale_unit', 'gross_weight', 'tare_weight', 'ship_weight',
                    'batch_managed', 'default_warehouse', 'last_sap_update',
                    'signal_word', 'picto1', 'picto2', 'picto3', 'picto4']
            ...
            for r in rows:
                ...
                row_dict['last_sap_update'] = (
                    upd.strftime('%Y-%m-%d') if hasattr(upd, 'strftime') else str(upd or '')
                )
        ...
        return pd.read_sql(query, getattr(self._local, 'connection', None), params=[material_number])
    ...
    def execute_query(self, query: str, params: list = None) -> pd.DataFrame:
        ...
        return pd.read_sql(query, getattr(self._local, 'connection', None), params=params or [])
    ...
    def list_tables(self, schema: str = None) -> pd.DataFrame:
        ...
        return pd.read_sql(query, getattr(self._local, 'connection', None), params=[target_schema])
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._mock_orders = {
            '300050': {
                ...
                'items': [
                    {'item_number': '000010', 'material_number': 'IFF-QB00122', 
                     'description': '1181-D MENTA', 'quantity': 12, 'unit': 'KG', 'net_value': 8400.00},
                    {'item_number': '000020', 'material_number': 'VAR-QB00005',
                     'description': 'ACEITE DE PINO Y55', 'quantity': 5, 'unit': 'LT', 'net_value': 6600.00}
                ]
            }
        }
...
if __name__ == "__main__":
    ...
    if use_mock:
        ...
        prompt_user = f"SAP Username [{default_user}]: " if default_user else "SAP Username: "
