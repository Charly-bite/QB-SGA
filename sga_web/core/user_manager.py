...
class UserRole(Enum):
    ...
    OPERATOR = "operator"    # Can print labels and scan products, but NOT add/edit chemicals
...
class UserManager:
    ...
    def _ensure_defaults(self):
        ...
        if self.sql_engine is None:
            logger.error("SQL engine is not available — user authentication will not work!")
            return
        ...
        try:
            with self.sql_engine.begin() as conn:
                ...
                row = conn.execute(text(
                    "SELECT COUNT(*) FROM SGA_Users WHERE role = 'admin'"
                )).scalar()
                    ...
                    conn.execute(text("""
                        ...
                        VALUES 
                            (:u, :p, 'admin', 'Administrator', '', '',
                             :now, 1, 1)
                    """), {"u": "admin", "p": pwd_hash, "now": datetime.now().isoformat()})
        ...
        try:
            with self.sql_engine.connect() as conn:
                ...
                for row in result:
                    ...
                    if not user.get("email"): user["email"] = ""
                    if not user.get("warehouse"): user["warehouse"] = ""
    ...
    def _save_data(self, data: Dict[str, Any]):
        ...
        try:
            ...
            with self.sql_engine.begin() as conn:
                db_users = conn.execute(text("SELECT username FROM SGA_Users")).fetchall()
                ...
                for user in users_data:
                    ...
                    params = {
                        "username": username, "pwd": pwd, "role": role, "full_name": full_name,
                        "email": email, "warehouse": warehouse, "created_at": created_at,
                        "last_login": last_login, "is_active": is_active, "must_change": must_change
                    }
    ...
    def _verify_password(self, password: str, stored_hash: str) -> bool:
        ...
        try:
            ...
            if '$' not in stored_hash:
                return self._hash_password(password, "default_salt") == f"default_salt${stored_hash}"
    ...
    def authenticate(self, username: str, password: str) -> bool:
        ...
        for user in data.get("users", []):
            if user["username"].lower() == username.lower() and user.get("is_active", True):
    ...
    def _update_last_login(self, username: str):
        ...
        try:
            with self.sql_engine.begin() as conn:
                conn.execute(text(
                    "UPDATE SGA_Users SET last_login = :now WHERE username = :u"
    ...
    def has_permission(self, required_role: UserRole, requesting_user: Dict[str, Any] = None) -> bool:
    ...
    def create_user(self, username: str, password: str, role: UserRole, 
                   full_name: str = "", email: str = "", warehouse: str = "", requesting_user: Dict[str, Any] = None) -> tuple[bool, str]:
        """
    ...
    def update_user(self, username: str, requesting_user: Dict[str, Any] = None, **kwargs) -> tuple[bool, str]:
        """
        ...
        requester = requesting_user if requesting_user else (self._current_user if self.is_logged_in() else None)
        
        # Permission check
        is_self_password_change = (
            requester and 
            requester["username"].lower() == username.lower() and 
            len(kwargs) == 1 and 
            "password" in kwargs
        )
        
        if not is_self_password_change and not self.has_permission(UserRole.ADMIN, requesting_user):
            return False, "Permission denied: Admin access required"
        ...
        if "role" in kwargs and self.has_permission(UserRole.ADMIN, requesting_user):
            user["role"] = kwargs["role"].value if isinstance(kwargs["role"], UserRole) else kwargs["role"]
        ...
        if "is_active" in kwargs and self.has_permission(UserRole.ADMIN, requesting_user):
            user["is_active"] = kwargs["is_active"]
        
        self._save_data(data)
        
        # Update current user if it's self-update
        if self._current_user and self._current_user["username"].lower() == username.lower():
    ...
    def delete_user(self, username: str, requesting_user: Dict[str, Any] = None) -> tuple[bool, str]:
        ...
        try:
            with self.sql_engine.begin() as conn:
                result = conn.execute(text(
                    "DELETE FROM SGA_Users WHERE LOWER(username) = :u"
                ), {"u": username.lower()})
    ...
    def list_users(self, requesting_user: Dict[str, Any] = None) -> List[Dict[str, Any]]:
    ...
    def get_user_info(self, username: str = None, requesting_user: Dict[str, Any] = None) -> Optional[Dict[str, Any]]:
...
if __name__ == "__main__":
    ...
    if um.authenticate("admin", "admin123"):
        ...
        success, msg = um.create_user("operator1", "pass123", UserRole.OPERATOR, "Juan Pérez")
        print(f"  {msg}")
        
        success, msg = um.create_user("viewer1", "pass123", UserRole.VIEWER, "María López")
        print(f"  {msg}")
