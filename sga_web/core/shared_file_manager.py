...
class SharedFileManager:
    ...
    @contextmanager
    def lock_file(self, filepath: str, exclusive: bool = False, timeout: Optional[float] = None):
        """
        ...
        try:
            ...
            while True:
                try:
                    if os.name == 'nt':
                        ...
                        if not exclusive: file_obj.seek(0)
                    else:
            ...
            logger.debug(f"Acquired {'exclusive' if exclusive else 'shared'} lock on {filepath}")
            yield file_obj
