...
class PrintQueue:
    ...
    def get_pending(self) -> List[QueueItem]:
        ...
        return [i for i in self.items if i.status in [ItemStatus.PENDING.value, ItemStatus.MATCHED.value]]
...
class SGAController:
    ...
    def connect_sap(self, username: str = None, password: str = None) -> bool:
        ...
        if not user and not pwd and (not self.sap_connector.username or not self.sap_connector.password):
    ...
    def queue_order(self, order_number: int) -> List[QueueItem]:
        ...
        try:
            ...
            if not order_data:
                logger.warning(f"Order {order_number} not found in SAP or error fetching")
                return []
            
            header = order_data['header']
            items = order_data['items']
            
            logger.info(f"Order {order_number}: {header['customer_name']} - {len(items)} items")
    ...
    def generate_label(self, item: QueueItem, batch_number: str = None) -> Optional[str]:
        """
        ...
        if not item.ghs_matched:
            logger.warning(f"Cannot generate label for unmatched item: {item.item_code}")
            return None
    ...
    def print_selected(self, indices: List[int], batch_number: str = None) -> Dict[str, Any]:
        """
    ...
    def get_queue_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'no_ghs_data': len([i for i in items if i.status == ItemStatus.NO_GHS_DATA.value]),
    ...
    def display_queue(self):
        ...
        print("=" * 100)
        print(f"{'#':<4} {'Order':<8} {'Item Code':<15} {'Description':<30} {'Qty':<8} {'Status':<12} {'GHS':<5}")
        print("-" * 100)
        ...
        for i, item in enumerate(pending, 1):
            ...
            print(f"{i:<4} {item.order_number:<8} {item.item_code:<15} {desc:<30} {item.quantity:<8} {status:<12} {ghs:<5}")
    ...
    def clear_printed(self):
        ...
        self.queue.items = [i for i in self.queue.items if i.status != ItemStatus.PRINTED.value]
        self._save_queue()
