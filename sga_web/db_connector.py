    ...
    try:
        ...
        if result.returncode == 0:
            ...
            if "1219" in result.stderr:
                ...
                subprocess.run(f'net use "{share_path}" /delete /y', shell=True, timeout=10)
...
def mount_share(config):
    ...
    if ip_address:
        ...
        if ping_server(ip_address):
            ...
            print(f"[WARN] Server {ip_address} did not respond to ping (might be unreachable or firewall blocking)")
    ...
    if ip_address and share_name:
        print("[WARN] Hostname connection failed/skipped. Trying IP address fallback...")
