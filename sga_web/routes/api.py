...
def get_stats():
    ...
    stats = {
        'products': {
            'total': len(smart_label.df_products) if smart_label.df_products is not None else 0
        },
        'orders': {
            'total': len(order_mgr.orders),
            'by_status': {}
        },
        'sap': {
            'available': current_app.sap_available,
            'connected': current_app.sap_connector is not None and getattr(current_app.sap_connector, 'connected', False)
        }
    }
    
    # Count by status
    for status in OrderStatus:
        count = len([o for o in order_mgr.orders.values() if o.get('status') == status.value])
...
def get_pictogram(name):
    ...
    pictogram_path = os.path.join(current_app.config['PICTOGRAMS_PATH'], f'{file_name}.png')
