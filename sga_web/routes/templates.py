...
def _build_barcode_preview_svg(value: str, width_mm: float, bar_height_mm: float, bar_width: float, auto_fit: bool):
    ...
    if auto_fit and field_w_mm > 0 and barcode.width > 0:
        ...
        if barcode.width > available_w_pts:
            ...
            while barcode.width > available_w_pts and attempt_bw > 0.3:
                ...
                barcode = code128.Code128(txt_str, barHeight=bar_h_pts, barWidth=final_bar_w)
...
def designer():
    ...
    return render_template('templates/designer.html', template_data=None, available_fields=fields, user_warehouse=user_warehouse)
...
def designer_edit(template_id):
    ...
    fields = current_app.template_manager.get_available_fields()
    user_warehouse = getattr(current_user, 'warehouse', '') or ''
    return render_template('templates/designer.html', template_data=template_data, available_fields=fields, user_warehouse=user_warehouse)
...
def api_save():
    ...
    if not current_user.is_admin():
        return jsonify({'error': 'Solo administradores pueden gestionar plantillas'}), 403
...
def api_delete(template_id):
    """Delete a template"""
    if not current_user.is_admin():
        return jsonify({'error': 'Solo administradores pueden eliminar plantillas'}), 403
...
def api_duplicate(template_id):
    ...
    if not current_user.is_admin():
        return jsonify({'error': 'Solo administradores pueden duplicar plantillas'}), 403
...
def api_product_data(product_id):
    ...
    try:
        ...
        h_text = '\n'.join(
            f"{h_codes[i]} - {h_stmts[i]}" if i < len(h_stmts) and h_stmts[i] else h_codes[i]
            for i in range(len(h_codes))
        )
        p_text = '\n'.join(
            f"{p_codes[i]} - {p_stmts[i]}" if i < len(p_stmts) and p_stmts[i] else p_codes[i]
            for i in range(len(p_codes))
        )

        signal = str(product.get('signal_word', '')).upper()
        is_no_ghs = signal in ('NAN', 'NONE', 'NO APLICABLE', 'NO APLICA', 'N/A', '-', '')
        if is_no_ghs:
            signal = ''
        ...
        cas_text = f"CAS No. {product['cas']}" if product.get('cas') else 'CAS No. Mezcla'
        ...
        one_year = datetime(datetime.now().year + 1, datetime.now().month, datetime.now().day).strftime('%d/%m/%Y')
        ...
        return jsonify({
            ...
            'product_name_display': product.get('product_name') or product.get('name', ''),
            'fields': {
...
def api_barcode_preview():
    ...
    try:
        ...
        preview = _build_barcode_preview_svg(value, width_mm, bar_height_mm, bar_width, auto_fit)
...
def test_print_template(template_id):
    ...
    try:
        ...
        render_dpi_request = int(data.get('render_dpi', 200))  # allow caller to request higher DPI
        ...
        if generator is None:
            ...
            db_path = current_app.config.get('UNIFIED_DB_PATH', os.path.join(project_root, 'unified_db'))
        ...
        if output_format.startswith('image'):
            logging.info(f"Generating image preview. Format: {output_format}, Scale: {scale_percent}%")
            try:
                ...
                images = convert_from_path(output_path, poppler_path=poppler_path, dpi=render_dpi_request)
                ...
                if images:
                    ...
                    import time
                    ts = int(time.time())
                    img_filename = f"test_print_{template_id}_{product_id}_{ts}.{img_ext}"
                    ...
                    if scale_percent != 100:
                        ...
                        render_dpi = int(render_dpi_request * (100 / float(scale_percent)))
                ...
                import traceback
                traceback.print_exc()
                return jsonify({'error': f'Falló la conversión a imagen: {str(e)}'}), 500
        else:
