...
class SmartLabelManager:
    ...
    def _mark_local_pending_sync(self, changed_files: List[str]):
        ...
        if os.path.exists(marker_path):
            try:
                ...
                payload['changed_files'] = sorted(set(payload['changed_files'] + existing))
            except Exception:
                pass
    ...
    def _load_data(self):
        ...
        if getattr(self, 'sql_engine', None) is not None:
            try:
                ...
                self.df_products = pd.read_sql("SELECT * FROM products_master", con=self.sql_engine)
                ...
                try:
                    self.df_product_pictograms = pd.read_sql("SELECT * FROM product_pictograms", con=self.sql_engine)
                except Exception:
                    self.df_product_pictograms = None

                try:
                    self.df_product_hazards = pd.read_sql("SELECT * FROM product_hazards", con=self.sql_engine)
                except Exception:
                    self.df_product_hazards = None

                try:
                    self.df_product_precautions = pd.read_sql("SELECT * FROM product_precautions", con=self.sql_engine)
                except Exception:
        ...
        if os.path.exists(products_master_path) and os.path.exists(h_statements_path):
            ...
            self.df_products = pd.read_csv(products_master_path, dtype={'product_id': str})
            ...
            product_precautions_path = os.path.join(self.data_dir, "product_precautions.csv")
            product_pictograms_path = os.path.join(self.data_dir, "product_pictograms.csv")

            if os.path.exists(product_hazards_path):
                self.df_product_hazards = pd.read_csv(product_hazards_path, dtype={'product_id': str, 'h_code': str})
            else:
            ...
            if os.path.exists(product_precautions_path):
                self.df_product_precautions = pd.read_csv(
                    product_precautions_path, dtype={
                        'product_id': str, 'p_code': str})
            else:
                self.df_product_precautions = pd.DataFrame(columns=['product_id', 'p_code'])

            if os.path.exists(product_pictograms_path):
                self.df_product_pictograms = pd.read_csv(
                    product_pictograms_path, dtype={
                        'product_id': str, 'pictogram_id': str})
            else:
                self.df_product_pictograms = pd.DataFrame(columns=['product_id', 'pictogram_id'])
            ...
            print(f"Loaded {len(self.df_products)} products (Normalized), {len(self.h_defs)} H-defs, {len(self.p_defs)} P-defs.")
            ...
            print(f"Loaded {len(self.df_products)} products (Legacy), {len(self.h_defs)} H-defs, {len(self.p_defs)} P-defs.")
    ...
    def get_pictogram_path(self, picto_name: str) -> Optional[str]:
        ...
        assets_dir = os.path.join(os.path.dirname(self.data_dir.rstrip(os.sep)), "assets", "pictograms")
    ...
    def _create_lookup_dict(self, df: pd.DataFrame, key_col: str, val_col: str) -> Dict[str, str]:
    ...
    def resolve_code(self, code: str, code_type: str) -> str:
        ...
        if matches and len(matches) > 1:
            ...
            for part in matches:
                ...
                if part_text == part:  # Only returned the code itself, meaning not found
    ...
    def _save_products_normalized(self) -> bool:
        ...
        try:
            ...
            clean_df = self.df_products[self.df_products['product_id'].astype(
                str).str.strip().ne('') & self.df_products['product_id'].notna()]
    ...
    def update_product(self, product_id: str, data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            # Identify pictograms
            pictograms = []
            possible_pictograms = ['Exclamación', 'Corrosión', 'Peligro para la salud', 'Calavera',
                                   'Llama', 'Ambiente', 'Llama sobre círculo', 'Bomba explotando', 'Cilindro de gas']
            ...
            if self.is_normalized:
                return self._update_product_normalized(
                    product_id, chemical_name, cas_number, signal_word, emergency_phone, h_codes, p_codes, pictograms)
            else:
                return self._update_product_legacy(product_id, chemical_name, cas_number,
                                                   signal_word, emergency_phone, h_raw, p_raw, pictograms)
    ...
    def _update_product_normalized(self, product_id, name, cas, signal, emergency, h_codes, p_codes, pictograms):
        name_to_id_map = {
        ...
        if getattr(self, 'sql_engine', None) is not None:
            ...
            try:
                with self.sql_engine.begin() as conn:
                    # Update Master
                    conn.execute(
                        text("UPDATE products_master SET chemical_name=:name, cas_number=:cas, signal_word=:signal, emergency_phone=:emergency WHERE product_id=:pid"),
                        {"name": name or "", "cas": cas or "", "signal": signal or "",
                            "emergency": emergency or "", "pid": str(product_id)}
                    )
                    # Recreate relations via delete + insert
                    conn.execute(text("DELETE FROM product_hazards WHERE product_id=:pid"), {"pid": str(product_id)})
                    for hc in h_codes:
                        conn.execute(
                            text("INSERT INTO product_hazards (product_id, h_code) VALUES (:pid, :code)"), {
                    ...
                    for pc in p_codes:
                        conn.execute(
                            text("INSERT INTO product_precautions (product_id, p_code) VALUES (:pid, :code)"), {
                                "pid": str(product_id), "code": str(pc)})

                    conn.execute(text("DELETE FROM product_pictograms WHERE product_id=:pid"), {"pid": str(product_id)})
                    for pic in pic_ids:
                        conn.execute(
                            text("INSERT INTO product_pictograms (product_id, pictogram_id) VALUES (:pid, :code)"), {
        ...
        # 2. Update Hazards
        # Remove old
        self.df_product_hazards = self.df_product_hazards[self.df_product_hazards['product_id'].astype(
            str) != str(product_id)]
        # Add new
        new_hazards = pd.DataFrame({'product_id': [str(product_id)] * len(h_codes), 'h_code': h_codes})
        self.df_product_hazards = pd.concat([self.df_product_hazards, new_hazards], ignore_index=True)

        # 3. Update Precautions
        self.df_product_precautions = self.df_product_precautions[self.df_product_precautions['product_id'].astype(
            str) != str(product_id)]
        new_precautions = pd.DataFrame({'product_id': [str(product_id)] * len(p_codes), 'p_code': p_codes})
        self.df_product_precautions = pd.concat([self.df_product_precautions, new_precautions], ignore_index=True)

        # 4. Update Pictograms
        new_pictograms = pd.DataFrame({'product_id': [str(product_id)] * len(pic_ids), 'pictogram_id': pic_ids})
        self.df_product_pictograms = pd.concat([self.df_product_pictograms, new_pictograms], ignore_index=True)

        return self.save_products()

    def _update_product_legacy(self, code, name, cas, signal, emergency, h_raw, p_raw, pictograms):
        idx = self.df_products[self.df_products['Codigo interno'].astype(str) == str(code)].index
        if len(idx) == 0:
            return False
        ...
        # Reset pictograms
        possible_cols = ['Exclamación', 'Corrosión', 'Peligro para la salud', 'Calavera',
                         'Llama', 'Ambiente', 'Llama sobre círculo', 'Bomba explotando', 'Cilindro de gas']

        for col in possible_cols:
            full_col_name = f"{col} (PICTOGRAMA )"
            if full_col_name in self.df_products.columns:
                self.df_products.loc[i, full_col_name] = 'X' if col in pictograms else ''
    ...
    def add_product(self, product_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if self.is_normalized:
                return self._add_product_normalized(code, name, cas, signal, emergency, h_codes, p_codes, pictograms)
            else:
    ...
    def _add_product_normalized(self, code, name, cas, signal, emergency, h_codes, p_codes, pictograms):
        name_to_id_map = {
            'Bomba explotando': 'GHS01',
            'Llama': 'GHS02',
            'Llama sobre círculo': 'GHS03',
        ...
        if getattr(self, 'sql_engine', None) is not None:
            ...
            try:
                with self.sql_engine.begin() as conn:
                    ...
                    conn.execute(
                        text("INSERT INTO products_master (product_id, chemical_name, cas_number, signal_word, emergency_phone, is_active, last_updated) VALUES (:pid, :name, :cas, :signal, :emergency, 'Y', :last_up)"),
                        {"pid": str(code), "name": name or "", "cas": cas or "", "signal": signal or "",
                         "emergency": emergency or "", "last_up": last_updated}
                    )
        ...
        # 2. Add Hazards
        if h_codes:
            new_hazards = pd.DataFrame({'product_id': [str(code)] * len(h_codes), 'h_code': h_codes})
        ...
        # 3. Add Precautions
        if p_codes:
            new_precautions = pd.DataFrame({'product_id': [str(code)] * len(p_codes), 'p_code': p_codes})
        ...
        if pic_ids:
            new_pictograms = pd.DataFrame({'product_id': [str(code)] * len(pic_ids), 'pictogram_id': pic_ids})
            if self.df_product_pictograms is None:
                self.df_product_pictograms = new_pictograms
            else:
                self.df_product_pictograms = pd.concat([self.df_product_pictograms, new_pictograms], ignore_index=True)

        print(f"DEBUG: Added normalized product {code}. Master rows: {len(self.df_products)}")
        return self.save_products()
    ...
    def _delete_product_normalized(self, product_id: str) -> bool:
                ...
                with self.sql_engine.begin() as conn:
                    ...
                    conn.execute(
                        text("DELETE FROM product_precautions WHERE product_id=:pid"), {
                            "pid": str(product_id)})
                    conn.execute(text("DELETE FROM product_pictograms WHERE product_id=:pid"), {"pid": str(product_id)})
                    conn.execute(text("DELETE FROM products_master WHERE product_id=:pid"), {"pid": str(product_id)})
                return True
            except Exception as e:
        ...
        if self.df_product_precautions is not None and not self.df_product_precautions.empty:
        ...
        if self.df_product_pictograms is not None and not self.df_product_pictograms.empty:
            self.df_product_pictograms = self.df_product_pictograms[
                self.df_product_pictograms['product_id'].astype(str) != str(product_id)
            ].reset_index(drop=True)

        print(f"DEBUG: Deleted normalized product {product_id}. Remaining: {len(self.df_products)}")
        return self.save_products()

    def _delete_product_legacy(self, product_id: str) -> bool:
        """Delete product from legacy database."""
        mask = self.df_products['Codigo interno'].astype(str).str.strip() == str(product_id).strip()
        if not mask.any():
            print(f"Product {product_id} not found for deletion (legacy).")
            return False

        self.df_products = self.df_products[~mask].reset_index(drop=True)
        print(f"DEBUG: Deleted legacy product {product_id}. Remaining: {len(self.df_products)}")
        return self.save_products()
    ...
    def _get_product_data_normalized(self, keyword_or_id: str) -> Optional[Dict[str, Any]]:
        ...
        if getattr(self, 'sql_engine', None) is not None:
            ...
            try:
                ...
                df_h = pd.read_sql(
                    ...
                    con=self.sql_engine, params={'pid': product_id}
                )
                h_codes_list = df_h['h_code'].astype(str).tolist() if not df_h.empty else []

                # 3. Fetch P-codes
                df_p = pd.read_sql(
                    text("SELECT p_code FROM product_precautions WHERE product_id = :pid"),
                    con=self.sql_engine, params={'pid': product_id}
                )
                p_codes_list = df_p['p_code'].astype(str).tolist() if not df_p.empty else []

                # 4. Fetch Pictograms
                df_pic = pd.read_sql(
                    text("SELECT pictogram_id FROM product_pictograms WHERE product_id = :pid"),
                    con=self.sql_engine, params={'pid': product_id}
                )
                picto_ids = df_pic['pictogram_id'].astype(str).tolist() if not df_pic.empty else []
        ...
        # --- CSV Fallback Data Load ---
            ...
            print(f"DEBUG: (Normalized CSV) searching for '{keyword_or_id}' in {len(self.df_products)} rows")
            mask = (self.df_products['product_id'].astype(str) == keyword_or_id)
            if 'chemical_name' in self.df_products.columns:
                mask = mask | (
                    self.df_products['chemical_name'].astype(str).str.contains(
                        keyword_or_id, case=False, na=False, regex=False))
            ...
            p_codes_list = []
            if self.df_product_precautions is not None and not self.df_product_precautions.empty:
                p_mask = self.df_product_precautions['product_id'].astype(str) == product_id
                p_codes_list = self.df_product_precautions[p_mask]['p_code'].tolist()

            picto_ids = []
            if self.df_product_pictograms is not None and not self.df_product_pictograms.empty:
                picto_mask = self.df_product_pictograms['product_id'].astype(str) == product_id
                picto_ids = self.df_product_pictograms[picto_mask]['pictogram_id'].tolist()

        # --- Shared Data Processing ---
        # Resolve H and P codes to text
        h_texts = [self.resolve_code(c, 'H') for c in h_codes_list if c and str(c).strip()]
        p_texts = [self.resolve_code(c, 'P') for c in p_codes_list if c and str(c).strip()]
    ...
    def _get_product_data_legacy(self, keyword_or_id: str) -> Optional[Dict[str, Any]]:
        ...
        mask = (self.df_products['Codigo interno'].astype(str).str.strip() == str(keyword_or_id).strip()) | \
               (self.df_products['Chemical Name'].astype(str).str.contains(
                   keyword_or_id, case=False, na=False, regex=False))
        ...
        h_raw = str(row['H-Statements']).split(',') if pd.notna(row['H-Statements']) else []
        h_texts = [self.resolve_code(c, 'H') for c in h_raw if c.strip() and c.strip().lower() != 'nan']

        # Process P-Statements
        p_raw = str(row['Consejos (Frases P)']).split(',') if pd.notna(row['Consejos (Frases P)']) else []
        p_texts = [self.resolve_code(c, 'P') for c in p_raw if c.strip() and c.strip().lower() != 'nan']

        # Raw codes (clean whitespace)
        h_codes_list = [c.strip() for c in h_raw if c.strip() and c.strip().lower() != 'nan']
        p_codes_list = [c.strip() for c in p_raw if c.strip() and c.strip().lower() != 'nan']

        # Gather Pictograms (columns with 'X')
        pictograms = []
        possible_picto_cols = ['Exclamación', 'Corrosión', 'Peligro para la salud', 'Calavera',
                               'Llama', 'Ambiente', 'Llama sobre círculo', 'Bomba explotando', 'Cilindro de gas']
...
if __name__ == "__main__":
    ...
    for code, ctype in test_cases:
        res = manager.resolve_code(code if code != "Long instruction text..." else long_text, ctype)
