...
OLD_PROD_CLASSIFICATIONS = r'C:\Users\QB_DESARROLLO\Desktop\SGAv1.01\unified_db\product_classifications.json'
...
def main():
    ...
    with engine.connect() as conn:
        with conn.begin():
            ...
            res = conn.execute(text("SELECT product_id, lote_history FROM product_classifications"))
            all_current = {}
            ...
            res = conn.execute(text("SELECT product_id, event_date, new_lote FROM product_lote_history"))
            ...
            print(f"Pre-loaded {len(all_current)} products from SQL, {len(existing_sql_fps)} existing SQL history records")
            ...
            for pid, old_entries in old_history_map.items():
                ...
                merged_history.sort(
                    key=lambda x: normalize_ts(x.get('date') or x.get('timestamp') or ''),
                    reverse=True
                )
                ...
                if product_exists:
                    conn.execute(
                        text("UPDATE product_classifications SET lote_history=:hist WHERE product_id=:pid"),
                        {"hist": json.dumps(merged_history), "pid": pid}
                    )
                else:
                    old_data = old_classifications.get(pid, {})
                    conn.execute(
                        text("""INSERT INTO product_classifications 
                ...
                for entry in new_entries:
                    ...
                    conn.execute(
                        text("""INSERT INTO product_lote_history 
